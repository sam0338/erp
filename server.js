...
function initSchema() {
  db.exec(`
    ...
    -- ===== STORE REQUISITION =====
    -- Distinct from an Indent: a requisition asks the store for something
    -- that (as far as the requester knows) might already be sitting in
    -- stock — no vendor, no procurement, just "give me what's on the
    -- shelf." Only the shortfall — what the storekeeper genuinely can't
    -- hand over — ever becomes an Indent, and only for that shortfall
    -- quantity, not the whole request.
    CREATE TABLE IF NOT EXISTS requisitions (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      requisition_number TEXT UNIQUE NOT NULL,
      requisition_date DATETIME DEFAULT CURRENT_TIMESTAMP,
      area_of_use TEXT NOT NULL,
      priority TEXT DEFAULT 'Normal',
      notes TEXT,
      status TEXT DEFAULT 'Draft', -- Draft, Submitted, Processing, Completed, Cancelled
      requested_by INTEGER NOT NULL,
      reviewed_by INTEGER,
    ...
    CREATE TABLE IF NOT EXISTS requisition_items (
      ...
      unit_of_measure TEXT,
      status TEXT DEFAULT 'Pending', -- Pending, Issued, Partially Issued, Sent to Indent
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  db.exec(`
    ...
    -- ===== GST e-INVOICE / e-WAY BILL (via a licensed GSP) =====
    -- Every real integration for third-party software goes through a
    -- licensed GST Suvidha Provider (GSP) rather than direct government
    -- access — this holds ONE client's GSP account credentials. Like
    -- Google Drive, this can't be provisioned on a client's behalf; they
    -- (or you, on their behalf) sign up with a GSP of choice and enter
    -- the resulting credentials here.
    CREATE TABLE IF NOT EXISTS gst_gsp_settings (
      id INTEGER PRIMARY KEY CHECK (id = 1),
      provider_name TEXT,
  ...
  // ===== Round 29: Store Requisition -> Indent traceability =====
  // When a requisition item can't be fully covered from store stock, the
  // shortfall becomes a real indent_items row — this points back at
  // exactly which requisition line it came from, so "why does this indent
  // exist" always has a clear answer.
  ensureColumn('indent_items', 'source_requisition_item_id', 'INTEGER');
  // ===== Round 30: item-wise stock ledger needs to know which warehouse
  // actually fulfilled a requisition line — the /issue route took
  // warehouse_id but never kept it anywhere after using it.
  ensureColumn('requisition_items', 'issued_from_warehouse_id', 'INTEGER');
  // ===== Round 31: e-Invoice (IRN) + e-Way Bill via GSP =====
  ensureColumn('sales_invoices', 'irn', 'TEXT');
  ensureColumn('sales_invoices', 'irn_ack_number', 'TEXT');
  ensureColumn('sales_invoices', 'irn_ack_date', 'DATETIME');
  ensureColumn('sales_invoices', 'irn_signed_qr', 'TEXT');
  ensureColumn('sales_invoices', 'einvoice_status', "TEXT DEFAULT 'Not Generated'"); // Not Generated, Generated, Cancelled, Failed
  ensureColumn('sales_invoices', 'einvoice_error', 'TEXT');
  ensureColumn('sales_invoices', 'einvoice_cancelled_at', 'DATETIME');
  // Kept separate from the pre-existing manual `eway_bill_number` field on
  // delivery_challans (someone typing in a number they generated by hand
  // on the portal) — these are specifically for numbers this app
  // generated itself via the API.
  ensureColumn('delivery_challans', 'ewb_generated_number', 'TEXT');
  ensureColumn('delivery_challans', 'ewb_valid_until', 'DATETIME');
  ensureColumn('delivery_challans', 'ewb_status', "TEXT DEFAULT 'Not Generated'");
  ensureColumn('delivery_challans', 'ewb_error', 'TEXT');
  ensureColumn('delivery_challans', 'ewb_cancelled_at', 'DATETIME');
  // ===== Round 33: GRN totals never included tax; historical GST backfill =====
  // grn.total_amount was always qty * unit_price only — the "value of
  // what was received" was quietly basic-value-only everywhere it showed
  // up (Accounts' GRN submission list, GRN print). Adding the tax-inclusive
  // figure alongside it rather than changing what total_amount means,
  // since other code may already depend on it staying basic-value.
  ensureColumn('grn', 'tax_amount', 'DECIMAL(12, 2) DEFAULT 0');
  ensureColumn('grn', 'grand_total', 'DECIMAL(12, 2) DEFAULT 0');
...
const MODULES = ['dashboard', 'purchase', 'indent', 'requisition', 'inventory', 'quality', 'production', 'sales', 'pos', 'accounts', 'finance', 'expenses', 'reports', 'masters', 'admin'];
const DEFAULT_MODULE_PERMISSIONS = {
  admin: MODULES.reduce((acc, key) => ({ ...acc, [key]: true }), {}),
  approver: { dashboard: true, purchase: true, indent: true, requisition: true, inventory: true, quality: true, production: true, sales: true, pos: false, finance: false, expenses: true, reports: true, masters: false, admin: false },
  purchaser: { dashboard: true, purchase: true, indent: true, requisition: true, inventory: true, quality: false, production: false, sales: false, pos: false, finance: false, expenses: true, reports: false, masters: true, admin: false },
  // Requisitions land with the storekeeper first (that's who physically
  // walks the shelf to confirm stock), so this is the one role that had
  // indent:false but genuinely needs requisition:true.
  storekeeper: { dashboard: true, purchase: false, indent: false, requisition: true, inventory: true, quality: true, production: false, sales: false, pos: false, finance: false, expenses: true, reports: false, masters: false, admin: false },
  production: { dashboard: true, purchase: false, indent: true, requisition: true, inventory: true, quality: false, production: true, sales: false, pos: false, finance: false, expenses: true, reports: false, masters: true, admin: false },
  sales: { dashboard: true, purchase: false, indent: false, requisition: false, inventory: true, quality: false, production: false, sales: true, pos: false, accounts: false, finance: false, expenses: true, reports: false, masters: true, admin: false },
  accounts: { dashboard: true, purchase: false, indent: false, requisition: false, inventory: false, quality: false, production: false, sales: true, pos: false, accounts: true, finance: true, expenses: true, reports: true, masters: false, admin: false },
  ...
  cashier: { dashboard: true, purchase: false, indent: false, requisition: false, inventory: true, quality: false, production: false, sales: false, pos: true, accounts: false, finance: false, expenses: true, reports: false, masters: false, admin: false },
  employee: { dashboard: true, purchase: false, indent: true, requisition: true, inventory: false, quality: false, production: false, sales: false, pos: false, finance: false, expenses: true, reports: false, masters: false, admin: false }
};
...
app.use('/api/requisition', verifyToken, requireModuleAccess('requisition'));
...
// ===== MATERIALS ROUTES =====
// Picking a material's name to request/indent something isn't a
// "purchase" action — it's basic informational lookup. Gating it behind
// the Purchase module (like /api/purchase/materials does) meant anyone
// without purchase access — a plain 'employee' role, for instance —
// couldn't even populate the material dropdown on Raise Indent or Raise
// Requisition. This sits outside any module's app.use(...) prefix, so
// only login is required, not a specific module permission.
app.get('/api/materials/lookup', verifyToken, (req, res) => {
  try {
    res.json(db.prepare(`
      SELECT id, material_code, material_name, unit_of_measure, category, item_type, hsn_code, gst_rate, barcode, default_sale_price, reorder_level
      FROM materials WHERE is_active = 1 ORDER BY material_name
    `).all() || []);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
  ...
  const createGRN = db.transaction((body, userId) => {
    ...
    for (const item of items) {
      ...
      if (poItem && receivedQty > 0) {
        const lineBase = receivedQty * poItem.unit_price;
        // BUG FIX: this used to only ever accumulate the basic value —
        // the "value of what was received" shown throughout Accounts (GRN
        // submissions list, GRN print) was quietly missing tax entirely,
        // even though the PO item it's costed from already has a real
        // tax_rate on it. tax_amount/grand_total are new, additive
        // columns — total_amount keeps its existing basic-value meaning
        // for anything already relying on it.
        total_amount += lineBase;
        tax_amount += lineBase * ((parseFloat(poItem.tax_rate) || 0) / 100);
    ...
    db.prepare('UPDATE grn SET total_amount = ?, tax_amount = ?, grand_total = ? WHERE id = ?').run(total_amount, Number(tax_amount.toFixed(2)), Number((total_amount + tax_amount).toFixed(2)), grnId);
...
app.get('/api/accounts/grn-submissions', verifyToken, (req, res) => {
  try {
    ...
    const rows = db.prepare(`
      SELECT s.*, g.grn_number, g.grn_date, g.total_amount, g.tax_amount, g.grand_total, po.po_number, v.vendor_name,
...
// ===== Round 29: Store Requisition =====
// The step that sits before an Indent: "does the store already have
// this?" A requisition asks the store directly — no vendor, no
// procurement — and only the genuine shortfall (what the storekeeper
// physically can't hand over) ever becomes an Indent, for exactly that
// shortfall quantity, with a traceable link back to the requisition line
// it came from.

function getRequisitionItems(requisitionId) {
  return db.prepare(`
    SELECT ri.*, m.material_name, m.material_code, m.unit_of_measure as material_uom,
           COALESCE((SELECT SUM(sl.quantity) FROM stock_ledger sl WHERE sl.material_id = ri.material_id), 0) AS quantity_available
...
// Recomputes the header status from where every item currently stands —
// called after both /issue and /convert-to-indent so the header never
// drifts out of sync with its lines.
function refreshRequisitionStatus(requisitionId) {
  const items = db.prepare('SELECT status FROM requisition_items WHERE requisition_id = ?').all(requisitionId);
  const allResolved = items.length > 0 && items.every(i => i.status === 'Issued' || i.status === 'Sent to Indent');
  const anyActioned = items.some(i => i.status !== 'Pending');
  const status = allResolved ? 'Completed' : (anyActioned ? 'Processing' : 'Submitted');
  db.prepare('UPDATE requisitions SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(status, requisitionId);
  return status;
}

const REQUISITION_SELECT = `
  SELECT r.*, u.full_name as requested_by_name, u.department as requested_from_dept,
         rv.full_name as reviewed_by_name,
         (SELECT COUNT(*) FROM requisition_items ri WHERE ri.requisition_id = r.id) as item_count,
         (SELECT GROUP_CONCAT(m.material_name, ', ') FROM requisition_items ri LEFT JOIN materials m ON ri.material_id = m.id WHERE ri.requisition_id = r.id) as item_summary
  FROM requisitions r
  LEFT JOIN users u ON r.requested_by = u.id
  LEFT JOIN users rv ON r.reviewed_by = rv.id
`;

app.get('/api/requisition/inventory/material/:materialId', verifyToken, (req, res) => {
  try {
    const row = db.prepare('SELECT COALESCE(SUM(quantity), 0) AS quantity_available FROM stock_ledger WHERE material_id = ?').get(req.params.materialId);
    res.json(row || { quantity_available: 0 });
  } catch (err) {
...
app.post('/api/requisition/create', verifyToken, (req, res) => {
  const create = db.transaction((body, userId) => {
    const { area_of_use, priority, notes, items = [] } = body;
    if (!items.length) throw new Error('Add at least one item to the requisition');
    for (const it of items) {
      if (!it.material_id) throw new Error('Every item needs a material selected');
      if (!parseFloat(it.quantity_requested) || parseFloat(it.quantity_requested) <= 0) throw new Error('Every item needs a quantity greater than 0');
    }
    const requisition_number = nextSerial('requisition');
    const result = db.prepare(
      'INSERT INTO requisitions (requisition_number, area_of_use, priority, notes, status, requested_by) VALUES (?, ?, ?, ?, ?, ?)'
    ).run(requisition_number, area_of_use, priority || 'Normal', notes || null, 'Draft', userId);
    const id = result.lastInsertRowid;
    const insertItem = db.prepare('INSERT INTO requisition_items (requisition_id, material_id, quantity_requested, unit_of_measure) VALUES (?, ?, ?, ?)');
    for (const it of items) {
      insertItem.run(id, it.material_id, parseFloat(it.quantity_requested), it.unit_of_measure || null);
    }
    return { requisition_number, id };
  });
  try {
    const { requisition_number, id } = create(req.body, req.userId);
    res.status(201).json({ requisition_number, id, message: 'Requisition created successfully' });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});

app.put('/api/requisition/:id', verifyToken, (req, res) => {
  const update = db.transaction((requisitionId, body) => {
    const requisition = db.prepare('SELECT * FROM requisitions WHERE id = ?').get(requisitionId);
    if (!requisition) throw new Error('Requisition not found');
    if (requisition.status !== 'Draft') throw new Error(`This requisition is ${requisition.status} and can no longer be edited — only Draft requisitions can be changed.`);
    const { area_of_use, priority, notes, items = [] } = body;
    if (!items.length) throw new Error('Add at least one item to the requisition');
    for (const it of items) {
      if (!it.material_id) throw new Error('Every item needs a material selected');
    ...
    db.prepare('UPDATE requisitions SET area_of_use = ?, priority = ?, notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
      .run(area_of_use, priority || 'Normal', notes || null, requisitionId);
    db.prepare('DELETE FROM requisition_items WHERE requisition_id = ?').run(requisitionId);
    const insertItem = db.prepare('INSERT INTO requisition_items (requisition_id, material_id, quantity_requested, unit_of_measure) VALUES (?, ?, ?, ?)');
    for (const it of items) {
      insertItem.run(requisitionId, it.material_id, parseFloat(it.quantity_requested), it.unit_of_measure || null);
    }
  });
  try {
    update(req.params.id, req.body);
...
app.get('/api/requisition/my-requisitions', verifyToken, (req, res) => {
  try {
    const requisitions = db.prepare(`${REQUISITION_SELECT} WHERE r.requested_by = ? ORDER BY r.created_at DESC`).all(req.userId);
...
app.put('/api/requisition/:id/submit', verifyToken, (req, res) => {
  try {
    const itemCount = db.prepare('SELECT COUNT(*) as c FROM requisition_items WHERE requisition_id = ?').get(req.params.id).c;
    if (!itemCount) return res.status(400).json({ error: 'This requisition has no items — add at least one before submitting.' });
    db.prepare("UPDATE requisitions SET status = 'Submitted', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(req.params.id);
    res.json({ message: 'Requisition submitted to the store' });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});

// The storekeeper/purchaser queue — anything not yet fully resolved.
app.get('/api/requisition/pending-review', verifyToken, (req, res) => {
  try {
    const requisitions = db.prepare(`${REQUISITION_SELECT} WHERE r.status IN ('Submitted', 'Processing') ORDER BY r.created_at ASC`).all();
    res.json(requisitions || []);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/requisition/:id', verifyToken, (req, res) => {
  try {
    const requisition = db.prepare(`${REQUISITION_SELECT} WHERE r.id = ?`).get(req.params.id);
    if (!requisition) return res.status(404).json({ error: 'Requisition not found' });
    requisition.items = getRequisitionItems(req.params.id);
...
// Storekeeper physically confirms stock and hands it over — this is what
// actually deducts from the system, same consumeStock() FIFO logic every
// other stock-out path in the app already uses.
app.post('/api/requisition/:id/issue', verifyToken, (req, res) => {
  const issue = db.transaction((requisitionId, body, userId) => {
    const { warehouse_id, items = [] } = body;
    if (!warehouse_id) throw new Error('Select which warehouse this is being issued from');
    if (!items.length) throw new Error('Nothing selected to issue');
    const requisitionItems = getRequisitionItems(requisitionId);

    for (const it of items) {
      const reqItem = requisitionItems.find(ri => ri.id === parseInt(it.requisition_item_id));
      if (!reqItem) throw new Error(`Item #${it.requisition_item_id} is not on this requisition`);
      const issueQty = parseFloat(it.quantity_issue) || 0;
      if (issueQty <= 0) continue;
      const remaining = reqItem.quantity_requested - reqItem.quantity_issued;
      if (issueQty > remaining + 0.001) throw new Error(`Cannot issue ${issueQty} of ${reqItem.material_name} — only ${remaining} is still outstanding on this requisition`);
      const ledger = db.prepare('SELECT quantity FROM stock_ledger WHERE material_id = ? AND warehouse_id = ?').get(reqItem.material_id, warehouse_id);
      if (!ledger || ledger.quantity < issueQty - 0.001) {
        throw new Error(`Insufficient physical stock for ${reqItem.material_name} in this warehouse: need ${issueQty}, have ${ledger ? ledger.quantity : 0}`);
      }

      consumeStock(reqItem.material_id, warehouse_id, issueQty);
      const newIssued = Number((reqItem.quantity_issued + issueQty).toFixed(2));
      const newStatus = newIssued >= reqItem.quantity_requested - 0.001 ? 'Issued' : 'Partially Issued';
      db.prepare('UPDATE requisition_items SET quantity_issued = ?, status = ?, issued_from_warehouse_id = ? WHERE id = ?').run(newIssued, newStatus, warehouse_id, reqItem.id);
    }

    db.prepare('UPDATE requisitions SET reviewed_by = ?, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?').run(userId, requisitionId);
    return refreshRequisitionStatus(requisitionId);
  });
  try {
    const status = issue(req.params.id, req.body, req.userId);
    res.json({ message: 'Stock issued successfully', status });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});

// The escalation path: whatever's still short after issuing becomes a
// real Indent — for the shortfall quantity only, not the original ask —
// and drops straight into the normal approval/RFQ/PO pipeline exactly
// like any other indent from here on.
app.post('/api/requisition/:id/convert-to-indent', verifyToken, (req, res) => {
  const convert = db.transaction((requisitionId, body, userId) => {
    const requisition = db.prepare('SELECT * FROM requisitions WHERE id = ?').get(requisitionId);
    if (!requisition) throw new Error('Requisition not found');
    const requisitionItems = getRequisitionItems(requisitionId);

    const { item_ids } = body; // optional explicit selection; defaults to every item with an outstanding shortfall
    const candidates = requisitionItems.filter(ri =>
      ri.status !== 'Sent to Indent' &&
      (ri.quantity_requested - ri.quantity_issued) > 0.001 &&
      (!item_ids || item_ids.map(Number).includes(ri.id))
    );
    if (!candidates.length) throw new Error('Nothing on this requisition currently has an outstanding shortfall to send to procurement');

    const setting = db.prepare("SELECT levels_required FROM approval_settings WHERE module = 'indent'").get();
    const requiredLevel = setting ? setting.levels_required : 1;

    const indent_number = nextSerial('indent');
    const indentResult = db.prepare(
      'INSERT INTO indents (indent_number, area_of_use, priority, justification, status, current_level, required_level, requested_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(indent_number, requisition.area_of_use, requisition.priority,
      `Auto-generated from Store Requisition ${requisition.requisition_number} — item(s) not available in store.`,
      'Pending', 0, requiredLevel, requisition.requested_by);
    const indentId = indentResult.lastInsertRowid;

    const insertIndentItem = db.prepare('INSERT INTO indent_items (indent_id, material_id, quantity_required, unit_of_measure, material_type, source_requisition_item_id) VALUES (?, ?, ?, ?, ?, ?)');
    for (const ri of candidates) {
      const shortfall = Number((ri.quantity_requested - ri.quantity_issued).toFixed(2));
      insertIndentItem.run(indentId, ri.material_id, shortfall, ri.unit_of_measure || ri.material_uom, 'Raw Material', ri.id);
      db.prepare("UPDATE requisition_items SET status = 'Sent to Indent' WHERE id = ?").run(ri.id);
    }

    db.prepare('UPDATE requisitions SET reviewed_by = ?, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?').run(userId, requisitionId);
    const status = refreshRequisitionStatus(requisitionId);

    return { indent_number, indent_id: indentId, requisitionStatus: status };
  });
  try {
    const result = convert(req.params.id, req.body, req.userId);
    res.status(201).json({ ...result, message: `Indent ${result.indent_number} created for the outstanding shortfall` });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
function getSalesOrderWithItems(id) {
  ...
  if (!so) return null;
  so.items = db.prepare(`
    SELECT soi.*, m.material_name, m.material_code, m.unit_of_measure as material_unit, m.hsn_code
...
function getInvoiceWithItems(id) {
  const invoice = db.prepare(`
    SELECT si.*, c.customer_name, c.address as customer_address, c.city as customer_city, c.state as customer_state,
           c.gst_number as customer_gstin, c.phone as customer_phone, c.postal_code as customer_pincode
...
// Real e-invoicing/e-way bill integration goes through a licensed GST
// Suvidha Provider (GSP) — this app can't reach the government IRP/EWB
// systems directly, the same way it couldn't provision a Google Cloud
// project for Drive backup. This module is built against the stable,
// long-documented CORE fields of the government e-invoice (INV-01) and
// e-way bill schemas — the field names/shapes that have been consistent
// across GSPs for years. But GSPs commonly wrap auth and the outer
// envelope slightly differently from each other. The parts most likely
// to need adjusting for your specific GSP: getGspAuthToken()'s auth call
// shape, and the exact response envelope callGsp() unwraps. The payload
// BODIES (DocDtls/SellerDtls/BuyerDtls/ItemList/ValDtls for e-invoice;
// the from/to/item fields for e-way bill) are the well-established
// government schema and are the least likely part to need changes.
//
// This has NOT been tested against a live GSP sandbox — that requires
// real API credentials and network access neither of which exist in the
// environment this was built in. Test thoroughly against your GSP's
// sandbox before anywhere near production data.

function getGstGspSettings() {
  return db.prepare('SELECT * FROM gst_gsp_settings WHERE id = 1').get();
}

// GSPs almost universally issue a short-lived bearer token from a
// client_id/client_secret (sometimes plus username/password) pair —
// cached here so every invoice doesn't re-authenticate.
async function getGspAuthToken() {
  const settings = getGstGspSettings();
  if (!settings || !settings.base_url || !settings.client_id) {
    throw new Error('GST e-Invoice/e-Way Bill is not configured yet. Set it up under System Settings → GST Compliance first.');
  }
  if (settings.cached_token && settings.token_expires_at && new Date(settings.token_expires_at) > new Date(Date.now() + 60000)) {
    return settings.cached_token;
  }
  const authUrl = `${settings.base_url.replace(/\/$/, '')}/auth`;
  ...
  if (!response.ok || !data.access_token) {
    throw new Error(`GSP authentication failed: ${data.message || data.error || response.statusText}`);
  }
  // Most GSPs return an expiry in seconds; default to a conservative 20
  // minutes if the response doesn't say, so a stale token never lingers.
  const expiresInMs = (data.expires_in ? data.expires_in * 1000 : 20 * 60 * 1000);
  db.prepare('UPDATE gst_gsp_settings SET cached_token = ?, token_expires_at = ? WHERE id = 1')
    .run(data.access_token, new Date(Date.now() + expiresInMs).toISOString());
  return data.access_token;
}

async function callGsp(path, body) {
  const settings = getGstGspSettings();
  const token = await getGspAuthToken();
  const url = `${settings.base_url.replace(/\/$/, '')}${path}`;
  const response = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`, 'gstin': settings.gstin || '' },
    body: JSON.stringify(body)
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok || data.status === 'FAILED' || data.error) {
    const message = (data.errors && JSON.stringify(data.errors)) || data.message || data.error || response.statusText;
    throw new Error(`GSP request failed: ${message}`);
  }
  return data;
}
...
const INDIAN_STATE_CODES = {
  ...
  'assam': '18', 'west bengal': '19', 'jharkhand': '20', 'odisha': '21', 'chattisgarh': '22', 'madhya pradesh': '23',
...
// Builds the core INV-01 e-invoice schema body from an already-generated
// SAKAAR invoice — every field here is computed from data that already
// exists (Company Settings, the customer record, the invoice + its
// lines), nothing new to enter.
function buildEinvoicePayload(invoice, company) {
  if (!company || !company.gstin) throw new Error('Company Settings has no GSTIN on file — required for e-invoicing.');
  if (!invoice.customer_gstin) throw new Error('This customer has no GSTIN on file — e-invoicing (B2B) requires one.');
  const items = invoice.items.map((it, idx) => ({
    SlNo: String(idx + 1),
    PrdDesc: it.description || it.material_name || 'Item',
    IsServc: 'N',
    HsnCd: it.hsn_code || '',
    Qty: parseFloat(it.quantity) || 0,
    Unit: (it.unit_of_measure || 'NOS').toUpperCase().slice(0, 8),
    UnitPrice: parseFloat(it.unit_price) || 0,
    TotAmt: Number((parseFloat(it.quantity) * parseFloat(it.unit_price)).toFixed(2)),
    Discount: parseFloat(it.discount_amount) || 0,
    AssAmt: parseFloat(it.taxable_value) || 0,
    GstRt: parseFloat(it.tax_rate) || 0,
    IgstAmt: parseFloat(it.igst_amount) || 0,
    CgstAmt: parseFloat(it.cgst_amount) || 0,
    SgstAmt: parseFloat(it.sgst_amount) || 0,
    TotItemVal: parseFloat(it.line_total) || 0
  }));
  return {
    Version: '1.1',
    TranDtls: { TaxSch: 'GST', SupTyp: 'B2B', RegRev: 'N', IgstOnIntra: 'N' },
    DocDtls: { Typ: 'INV', No: invoice.invoice_number, Dt: ddmmyyyy(invoice.invoice_date) },
    SellerDtls: {
      Gstin: company.gstin, LglNm: company.company_name, Addr1: company.address || '',
      Loc: company.city || '', Pin: parseInt(company.postal_code) || 0, Stcd: stateCode(company.state),
      Ph: company.phone || '', Em: company.email || ''
    },
    BuyerDtls: {
      Gstin: invoice.customer_gstin, LglNm: invoice.customer_name, Pos: stateCode(invoice.customer_state),
      Addr1: invoice.customer_address || '', Loc: invoice.customer_city || '',
      Pin: parseInt(invoice.customer_pincode) || 0, Stcd: stateCode(invoice.customer_state)
    },
    ItemList: items,
    ValDtls: {
      AssVal: parseFloat(invoice.taxable_value) || 0,
      CgstVal: parseFloat(invoice.cgst_total) || 0,
      SgstVal: parseFloat(invoice.sgst_total) || 0,
      IgstVal: parseFloat(invoice.igst_total) || 0,
      TotInvVal: parseFloat(invoice.grand_total) || 0
    }
  };
}

// Builds the core e-way bill "Generate EWB" schema body from a delivery
// challan — reuses whatever the linked Sales Order/customer/company
// already has on file.
function buildEwaybillPayload(challan, company) {
  if (!challan.vehicle_number && !challan.transporter) {
    throw new Error('Add a vehicle number or transporter to this challan before generating an e-Way Bill.');
  }
  const totalValue = (challan.items || []).reduce((sum, it) => sum + (parseFloat(it.taxable_value) || 0), 0);
  return {
    supplyType: 'O',
    subSupplyType: '1',
    docType: 'DCH', // Delivery Challan; use 'INV' if generating against a tax invoice instead
    docNo: challan.challan_number,
    docDate: ddmmyyyy(challan.challan_date),
    fromGstin: company.gstin,
    fromTrdName: company.company_name,
    fromAddr1: company.address || '',
    fromPlace: company.city || '',
    fromPincode: parseInt(company.postal_code) || 0,
    fromStateCode: stateCode(company.state),
    toGstin: challan.customer_gstin || 'URP',
    toTrdName: challan.customer_name || '',
    toAddr1: challan.customer_address || '',
    toPlace: challan.customer_city || challan.destination || '',
    toPincode: parseInt(challan.customer_pincode) || 0,
    toStateCode: stateCode(challan.customer_state),
    transactionType: 1,
    ...
    vehicleType: 'R',
    itemList: (challan.items || []).map(it => ({
      productName: it.material_name || 'Item',
      hsnCode: it.hsn_code || '',
      quantity: parseFloat(it.quantity) || 0,
      qtyUnit: (it.unit_of_measure || 'NOS').toUpperCase().slice(0, 8),
      taxableAmount: parseFloat(it.taxable_value) || 0
    }))
  };
}

app.get('/api/settings/gst-gsp', verifyToken, requireAdmin, (req, res) => {
  try {
    const settings = getGstGspSettings() || {};
    // Never send the secret/password back to the browser once saved.
    const { client_secret, password, cached_token, ...safe } = settings;
    res.json({ ...safe, has_credentials: !!(client_secret || password) });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.put('/api/settings/gst-gsp', verifyToken, requireAdmin, (req, res) => {
  try {
    const { provider_name, is_sandbox, base_url, client_id, client_secret, username, password, gstin } = req.body;
    if (!base_url || !client_id) return res.status(400).json({ error: 'At minimum, a base URL and Client ID are required' });
    db.prepare(`
      INSERT INTO gst_gsp_settings (id, provider_name, is_sandbox, base_url, client_id, client_secret, username, password, gstin, connected_at)
      VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
      ON CONFLICT(id) DO UPDATE SET provider_name=excluded.provider_name, is_sandbox=excluded.is_sandbox, base_url=excluded.base_url,
        client_id=excluded.client_id,
        client_secret = CASE WHEN excluded.client_secret != '' THEN excluded.client_secret ELSE gst_gsp_settings.client_secret END,
        username=excluded.username,
        password = CASE WHEN excluded.password != '' THEN excluded.password ELSE gst_gsp_settings.password END,
        gstin=excluded.gstin, cached_token=NULL, token_expires_at=NULL, updated_at=CURRENT_TIMESTAMP
    `).run(provider_name || null, is_sandbox ? 1 : 0, base_url, client_id, client_secret || '', username || null, password || '', gstin || null);
    res.json({ message: 'GST compliance settings saved' });
  } catch (err) { res.status(400).json({ error: err.message }); }
});

app.post('/api/settings/gst-gsp/test-connection', verifyToken, requireAdmin, async (req, res) => {
  try {
    await getGspAuthToken();
    res.json({ message: 'Connected successfully — credentials are valid and authentication works.' });
  } catch (err) { res.status(400).json({ error: err.message }); }
});

app.post('/api/finance/invoices/:id/generate-einvoice', verifyToken, async (req, res) => {
  try {
    const invoice = getInvoiceWithItems(req.params.id);
    if (!invoice) return res.status(404).json({ error: 'Invoice not found' });
    if (invoice.einvoice_status === 'Generated') return res.status(400).json({ error: 'An e-Invoice (IRN) already exists for this invoice.' });
    const company = db.prepare('SELECT * FROM company_settings WHERE id = 1').get();
    const payload = buildEinvoicePayload(invoice, company);
    const result = await callGsp('/einvoice/generate', payload);
    const irn = result.Irn || result.irn;
    const ackNo = result.AckNo || result.ackNo;
    const ackDt = result.AckDt || result.ackDt;
    const signedQr = result.SignedQRCode || result.signedQRCode;
    if (!irn) throw new Error('GSP response did not include an IRN — check the response shape matches your GSP\'s actual API.');
    db.prepare(`
      UPDATE sales_invoices SET irn = ?, irn_ack_number = ?, irn_ack_date = ?, irn_signed_qr = ?, einvoice_status = 'Generated', einvoice_error = NULL WHERE id = ?
    `).run(irn, ackNo || null, ackDt || null, signedQr || null, req.params.id);
    logAudit(req.userId, 'Generated e-Invoice', 'sales_invoice', req.params.id, null, { irn });
    res.json({ message: 'e-Invoice generated successfully', irn });
  } catch (err) {
    db.prepare("UPDATE sales_invoices SET einvoice_status = 'Failed', einvoice_error = ? WHERE id = ?").run(err.message, req.params.id);
    res.status(400).json({ error: err.message });
  }
});

app.post('/api/finance/invoices/:id/cancel-einvoice', verifyToken, requireAdmin, async (req, res) => {
  try {
    const invoice = db.prepare('SELECT * FROM sales_invoices WHERE id = ?').get(req.params.id);
    if (!invoice) return res.status(404).json({ error: 'Invoice not found' });
    if (invoice.einvoice_status !== 'Generated') return res.status(400).json({ error: 'No active e-Invoice to cancel on this invoice.' });
    const hoursOld = (Date.now() - new Date(invoice.irn_ack_date || invoice.created_at)) / 3600000;
    if (hoursOld > 24) throw new Error('This e-Invoice is more than 24 hours old — the government IRP only allows cancellation within 24 hours. Issue a credit note instead.');
    await callGsp('/einvoice/cancel', { Irn: invoice.irn, CnlRsn: '1', CnlRem: req.body.reason || 'Cancelled by user' });
    db.prepare("UPDATE sales_invoices SET einvoice_status = 'Cancelled', einvoice_cancelled_at = CURRENT_TIMESTAMP WHERE id = ?").run(req.params.id);
    logAudit(req.userId, 'Cancelled e-Invoice', 'sales_invoice', req.params.id, null, { irn: invoice.irn });
    res.json({ message: 'e-Invoice cancelled' });
  } catch (err) { res.status(400).json({ error: err.message }); }
});

app.post('/api/sales/challans/:id/generate-ewaybill', verifyToken, async (req, res) => {
  try {
    const challan = db.prepare(`
      SELECT dc.*, c.customer_name, c.address as customer_address, c.city as customer_city, c.state as customer_state,
             c.gst_number as customer_gstin, c.postal_code as customer_pincode
      FROM delivery_challans dc
      LEFT JOIN sales_orders so ON dc.so_id = so.id
      LEFT JOIN customers c ON so.customer_id = c.id
      WHERE dc.id = ?
    ...
    if (challan.ewb_status === 'Generated') return res.status(400).json({ error: 'An e-Way Bill already exists for this challan.' });
    challan.items = db.prepare('SELECT dci.*, m.material_name, m.hsn_code, m.gst_rate FROM delivery_challan_items dci LEFT JOIN materials m ON dci.material_id = m.id WHERE dci.challan_id = ?').all(challan.id);
    challan.items.forEach(it => { it.taxable_value = (parseFloat(it.quantity) || 0) * 0; }); // challan items carry no price — value comes from the linked invoice/SO when available
    const company = db.prepare('SELECT * FROM company_settings WHERE id = 1').get();
    const payload = buildEwaybillPayload(challan, company);
    const result = await callGsp('/ewaybill/generate', payload);
    const ewbNo = result.ewayBillNo || result.EwbNo;
    const validUpto = result.validUpto || result.ValidUpto;
    if (!ewbNo) throw new Error('GSP response did not include an e-Way Bill number — check the response shape matches your GSP\'s actual API.');
    db.prepare("UPDATE delivery_challans SET ewb_generated_number = ?, ewb_valid_until = ?, ewb_status = 'Generated', ewb_error = NULL WHERE id = ?")
      .run(ewbNo, validUpto || null, req.params.id);
    logAudit(req.userId, 'Generated e-Way Bill', 'delivery_challan', req.params.id, null, { ewb_number: ewbNo });
    res.json({ message: 'e-Way Bill generated successfully', ewb_number: ewbNo, valid_until: validUpto });
  } catch (err) {
    db.prepare("UPDATE delivery_challans SET ewb_status = 'Failed', ewb_error = ? WHERE id = ?").run(err.message, req.params.id);
    res.status(400).json({ error: err.message });
  }
});

app.post('/api/sales/challans/:id/cancel-ewaybill', verifyToken, requireAdmin, async (req, res) => {
  try {
    const challan = db.prepare('SELECT * FROM delivery_challans WHERE id = ?').get(req.params.id);
    if (!challan) return res.status(404).json({ error: 'Challan not found' });
    if (challan.ewb_status !== 'Generated') return res.status(400).json({ error: 'No active e-Way Bill to cancel on this challan.' });
    await callGsp('/ewaybill/cancel', { ewbNo: challan.ewb_generated_number, cancelRsnCode: '1', cancelRmrk: req.body.reason || 'Cancelled by user' });
    db.prepare("UPDATE delivery_challans SET ewb_status = 'Cancelled', ewb_cancelled_at = CURRENT_TIMESTAMP WHERE id = ?").run(req.params.id);
    logAudit(req.userId, 'Cancelled e-Way Bill', 'delivery_challan', req.params.id, null, { ewb_number: challan.ewb_generated_number });
    res.json({ message: 'e-Way Bill cancelled' });
  } catch (err) { res.status(400).json({ error: err.message }); }
});
...
// Backfills GST on historical Purchase Orders, GRNs, and Vendor Invoices
// that were saved with tax_rate/tax_amount = 0 before the GST calculation
// bugs were fixed — fixing the code going forward doesn't rewrite rows
// already sitting in the database. Only ever touches rows that are
// currently zero, so it's safe to run more than once: a second run finds
// nothing left to fix.
app.post('/api/finance/recalculate-gst', verifyToken, requireAdmin, (req, res) => {
  const recalc = db.transaction(() => {
    let itemsUpdated = 0, posRecalculated = 0, grnsRecalculated = 0, invoicesRecalculated = 0;

    // Step 1: backfill missing tax_rate on PO line items from the
    // material's current GST rate — the only place a "correct" rate can
    // plausibly be recovered from, since the original rate was simply
    // never captured for these older rows.
    const zeroTaxItems = db.prepare(`
      SELECT poi.id, poi.po_id, poi.quantity, poi.unit_price, m.gst_rate
      FROM purchase_order_items poi
      LEFT JOIN materials m ON poi.material_id = m.id
      WHERE (poi.tax_rate IS NULL OR poi.tax_rate = 0) AND m.gst_rate IS NOT NULL AND m.gst_rate > 0
    `).all();
    const updateItemTax = db.prepare('UPDATE purchase_order_items SET tax_rate = ?, line_total = ? WHERE id = ?');
    const touchedPoIds = new Set();
    for (const item of zeroTaxItems) {
      const qty = parseFloat(item.quantity) || 0;
      const price = parseFloat(item.unit_price) || 0;
      const rate = parseFloat(item.gst_rate) || 0;
      const lineTotal = Number((qty * price * (1 + rate / 100)).toFixed(2));
      updateItemTax.run(rate, lineTotal, item.id);
      touchedPoIds.add(item.po_id);
      itemsUpdated++;
    }

    // Step 2: recompute each affected PO's totals from its (now current)
    // items — the exact same math the PO creation route itself uses.
    const getPoItems = db.prepare('SELECT quantity, unit_price, tax_rate FROM purchase_order_items WHERE po_id = ?');
    const updatePoTotals = db.prepare('UPDATE purchase_orders SET total_amount = ?, tax_amount = ?, grand_total = ? WHERE id = ?');
    for (const poId of touchedPoIds) {
      ...
      for (const it of items) {
        ...
        total += base;
        tax += base * ((parseFloat(it.tax_rate) || 0) / 100);
      }
      updatePoTotals.run(Number(total.toFixed(2)), Number(tax.toFixed(2)), Number((total + tax).toFixed(2)), poId);
      posRecalculated++;
    }

    // Step 3: recompute GRNs linked to those POs — same reasoning as the
    // PO fix, GRN's own total was always basic-value-only if it predates
    // this round's GRN fix, or would still be zero-tax if it was created
    // from a PO whose items had no tax_rate at the time.
    const grnsToFix = db.prepare(`
      SELECT g.id, g.po_id FROM grn g
      WHERE g.po_id IN (${touchedPoIds.size ? [...touchedPoIds].map(() => '?').join(',') : 'NULL'})
         OR (g.tax_amount IS NULL OR g.tax_amount = 0)
    `).all(...touchedPoIds);
    const getGrnItems = db.prepare(`
      SELECT gi.received_quantity, poi.unit_price, poi.tax_rate
      FROM grn_items gi JOIN purchase_order_items poi ON gi.po_item_id = poi.id
      WHERE gi.grn_id = ?
    `);
    const updateGrnTotals = db.prepare('UPDATE grn SET total_amount = ?, tax_amount = ?, grand_total = ? WHERE id = ?');
    const seenGrn = new Set();
    for (const grn of grnsToFix) {
      if (seenGrn.has(grn.id)) continue;
      seenGrn.add(grn.id);
      const items = getGrnItems.all(grn.id);
      let total = 0, tax = 0;
      for (const it of items) {
        const base = (parseFloat(it.received_quantity) || 0) * (parseFloat(it.unit_price) || 0);
        total += base;
        tax += base * ((parseFloat(it.tax_rate) || 0) / 100);
      }
      if (total > 0) {
        updateGrnTotals.run(Number(total.toFixed(2)), Number(tax.toFixed(2)), Number((total + tax).toFixed(2)), grn.id);
        grnsRecalculated++;
      }
    }

    // Step 4: propagate to vendor invoices linked to those POs. If the
    // invoice's own recorded amount matches the PO's pre-tax total
    // closely, the PO's tax figure carries straight across; otherwise
    // (a partial invoice against a partial GRN) the PO's *effective tax
    // rate* is applied to whatever amount the invoice actually recorded,
    // which is the closest defensible reconstruction without per-line
    // detail on the invoice itself.
    const zeroTaxInvoices = db.prepare(`
      SELECT vi.id, vi.po_id, vi.total_amount, po.total_amount as po_total, po.tax_amount as po_tax
      FROM vendor_invoices vi
      JOIN purchase_orders po ON vi.po_id = po.id
      WHERE (vi.tax_amount IS NULL OR vi.tax_amount = 0) AND po.tax_amount > 0
    `).all();
    const updateInvoiceTax = db.prepare('UPDATE vendor_invoices SET tax_amount = ?, grand_total = ? WHERE id = ?');
    for (const inv of zeroTaxInvoices) {
      const invTotal = parseFloat(inv.total_amount) || 0;
      const poTotal = parseFloat(inv.po_total) || 0;
      const poTax = parseFloat(inv.po_tax) || 0;
      if (poTotal <= 0) continue;
      const closeMatch = Math.abs(invTotal - poTotal) < 0.01;
      const derivedTax = closeMatch ? poTax : Number((invTotal * (poTax / poTotal)).toFixed(2));
      updateInvoiceTax.run(derivedTax, Number((invTotal + derivedTax).toFixed(2)), inv.id);
      invoicesRecalculated++;
    }

    return { itemsUpdated, posRecalculated, grnsRecalculated, invoicesRecalculated };
  });

  try {
    const result = recalc();
    const total = result.itemsUpdated + result.posRecalculated + result.grnsRecalculated + result.invoicesRecalculated;
    res.json({
      message: total > 0
        ? `Recalculated: ${result.itemsUpdated} PO line item(s), ${result.posRecalculated} PO(s), ${result.grnsRecalculated} GRN(s), ${result.invoicesRecalculated} vendor invoice(s).`
        : 'Nothing to recalculate — no records currently have missing GST.',
      ...result
    });
  } catch (err) { res.status(400).json({ error: err.message }); }
});
...
// ===== Round 30: Store Issue Register + Item-wise Stock Ledger =====

// "Material issued from store to department" — every requisition line
// that's actually been handed over (fully or partially), with the
// approval/review trail, department, and material category alongside it
// in one row. Deliberately keyed off requisition_items.quantity_issued
// (not the header status) so a still-Processing requisition that's
// already issued *some* of its lines shows up correctly.
app.get('/api/mis/store-issue-register', verifyToken, (req, res) => {
  try {
    const { from, to, department } = req.query;
    const conditions = ['ri.quantity_issued > 0'];
    const params = [];
    if (from) { conditions.push('date(r.reviewed_at) >= date(?)'); params.push(from); }
    if (to) { conditions.push('date(r.reviewed_at) <= date(?)'); params.push(to); }
    if (department) { conditions.push('r.area_of_use = ?'); params.push(department); }
    const rows = db.prepare(`
      SELECT r.requisition_number, r.requisition_date, r.area_of_use as department, r.priority,
             m.material_name, m.material_code, m.category, m.item_type,
             ri.quantity_requested, ri.quantity_issued, ri.unit_of_measure, ri.status,
             u.full_name as requested_by_name, rv.full_name as approved_by_name, r.reviewed_at as approved_at,
             w.warehouse_name as issued_from_warehouse
      FROM requisition_items ri
      JOIN requisitions r ON ri.requisition_id = r.id
      LEFT JOIN materials m ON ri.material_id = m.id
      LEFT JOIN users u ON r.requested_by = u.id
      LEFT JOIN users rv ON r.reviewed_by = rv.id
      LEFT JOIN warehouses w ON ri.issued_from_warehouse_id = w.id
      WHERE ${conditions.join(' AND ')}
...
// A per-material "bin card": every inbound and outbound movement for one
// material, oldest first, with a running balance. Inbound movements all
// funnel through addStockBatch() regardless of source (GRN, Stock In,
// Production Receipt, Transfer In, Adjustment, ...), so stock_batches
// alone covers the "in" side. Nothing centralizes outbound movements the
// same way — each consuming flow (Stock Out, Sales Dispatch, Production
// issue, Store Requisition, Transfer Out, POS) keeps its own record — so
// the "out" side is a union across all of them.
app.get('/api/mis/item-ledger', verifyToken, (req, res) => {
  try {
    const { material_id, from, to } = req.query;
    if (!material_id) return res.status(400).json({ error: 'material_id is required' });

    const rows = db.prepare(`
      SELECT * FROM (
        SELECT sb.received_date as txn_date, sb.source as txn_type, sb.source_reference as reference,
               sb.quantity_received as qty_in, 0 as qty_out, w.warehouse_name
        FROM stock_batches sb LEFT JOIN warehouses w ON sb.warehouse_id = w.id
        WHERE sb.material_id = ?

        UNION ALL
        SELECT so.issued_date, 'Stock Out', so.stock_out_number, 0, so.quantity, w.warehouse_name
        FROM stock_out so LEFT JOIN warehouses w ON so.warehouse_id = w.id
        WHERE so.material_id = ?

        UNION ALL
        SELECT dc.challan_date, 'Sales Dispatch', dc.challan_number, 0, dci.quantity, w.warehouse_name
        FROM delivery_challan_items dci
        JOIN delivery_challans dc ON dci.challan_id = dc.id
        LEFT JOIN warehouses w ON dc.warehouse_id = w.id
        WHERE dci.material_id = ?

        UNION ALL
        SELECT pa.allocation_date, 'Issued to Production', pa.allocation_number, 0, pa.quantity_allocated, w.warehouse_name
        FROM production_allocation pa LEFT JOIN warehouses w ON pa.warehouse_id = w.id
        WHERE pa.material_id = ?

        UNION ALL
        SELECT r.reviewed_at, 'Issued to Dept (Requisition)', r.requisition_number, 0, ri.quantity_issued, w.warehouse_name
        FROM requisition_items ri
        JOIN requisitions r ON ri.requisition_id = r.id
        LEFT JOIN warehouses w ON ri.issued_from_warehouse_id = w.id
        WHERE ri.material_id = ? AND ri.quantity_issued > 0

        UNION ALL
        SELECT st.transfer_date, 'Transfer Out', st.transfer_number, 0, st.quantity, w.warehouse_name
        FROM stock_transfer st LEFT JOIN warehouses w ON st.from_warehouse_id = w.id
        WHERE st.material_id = ?

        UNION ALL
        SELECT si.invoice_date, 'POS Sale', si.invoice_number, 0, sii.quantity, w.warehouse_name
        FROM sales_invoice_items sii
        JOIN sales_invoices si ON sii.invoice_id = si.id
        LEFT JOIN warehouses w ON si.warehouse_id = w.id
        WHERE sii.material_id = ? AND si.channel = 'POS'
      )
      ORDER BY txn_date ASC
    `).all(material_id, material_id, material_id, material_id, material_id, material_id, material_id);

    // Running balance computed in JS rather than a SQL window function —
    // keeps this readable and doesn't depend on a specific SQLite build
    // supporting them, at the cost of one extra pass over what's usually
    // a small result set per material.
    let balance = 0;
    const withBalance = rows.map(r => {
      balance = Number((balance + (r.qty_in || 0) - (r.qty_out || 0)).toFixed(3));
      return { ...r, balance };
    });

    // Date filtering happens AFTER the running balance is computed, so a
    // row shown at the start of a filtered range still carries its true
    // historical balance rather than restarting from zero.
    const filtered = withBalance.filter(r => {
      if (from && r.txn_date < from) return false;
      if (to && r.txn_date > `${to} 23:59:59`) return false;
      return true;
    });
